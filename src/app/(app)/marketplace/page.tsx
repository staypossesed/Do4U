...
import {
  ...
  Sparkles, TrendingUp, Clock, Zap, Eye, Loader2,
} from "lucide-react";
...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      let q = supabase
        .from("listings")
        .select("id, title, price, category, status, views_count, created_at, listing_images(url_original, \"order\"), users(name, avatar_url)")
        .eq("status", "active")
  ...
  const displayed = [...listings].sort((a, b) => {
      ...
      <div className="px-4 pt-4 pb-2 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {geo.lat ? ` · ${geo.lat.toFixed(2)}, ${geo.lng?.toFixed(2)}` : ""}
          </p>
        </div>
          ...
          {displayed.length === 0 ? (
            ...
            >
              {displayed.map(listing => {
                const img = listing.listing_images?.sort((a, b) => a.order - b.order)?.[0]?.url_original;
                return (
                  <motion.div
                    key={listing.id}
